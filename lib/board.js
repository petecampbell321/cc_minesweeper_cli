  ...
  _createClass(Board, [{
    ...
    value: function getNumberOfNeighbourBombs(rowIndex, columnIndex) {
      ...
      neighbourOffsets.forEach(function (offset) {
        var neightbourRowIndex = rowIndex + offset[0];
        var neighbourColumnIndex = columnIndex + offset[1];
        if (neightbourRowIndex >= 0 && neightbourRowIndex < numberOfRows && neighbourColumnIndex >= 0 && neighbourColumnIndex < numberOfColumns) {
          if (_this._bombBoard[neightbourRowIndex][neighbourColumnIndex] === 'B') {
            numberOfBombs++;
          }
        }
      });
